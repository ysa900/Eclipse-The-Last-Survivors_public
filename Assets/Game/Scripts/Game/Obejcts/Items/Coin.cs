...
{
    public class Coin : Item, IPoolingObject
    {
        float followSpeed = 5f;

        public int coinAmount;
        private int stageEndCoinAmount;
        public int index;

        bool isInMagnetRange; // Coin이 자석 범위 안에 있나
        ...
        public void Init()
        {
            stageEndCoinAmount = coinAmount / 3;
            if (stageEndCoinAmount == 0) stageEndCoinAmount = 1;
        }
        ...
        void Update()
        {
            ...
            else // oldState == false, isInMagnetRange == false
            {
        ...
        private void OnTriggerEnter2D(Collider2D collision)
        {
            ...
            if (StageManager.instance.isStageClear) coinAmount = stageEndCoinAmount;

            iPlayer.GetCoin(coinAmount);

            AudioManager.instance.PlaySfx(AudioManager.Sfx.Pickup);
            PoolManager.instance.ReturnCoin(this, index);
        }
